...
const paginatedList = computed(() => {
  ...
  return submissionStore.filteredRecords.slice(start, end)
})

const total = computed(() => submissionStore.filteredRecords.length)

const typeOptions = computed(() => {
  const map = APPLICATION_TYPE_MAP as Record<string, string>
  return Object.entries(map).map(([value, label]) => ({ value, label }))
})

const statusOptions = computed(() => {
  const statusMap = APPLICATION_STATUS as Record<string, { label: string }>
  return Object.entries(statusMap).map(([value, config]) => ({ value, label: config.label }))
})
