...
const list = ref<CompetitionItem[]>([
  { id: '1', competitionName: '全国大学生数学建模竞赛', competitionType: 'national', awardLevel: 'second', awardDate: '2025-09', semester: '2023-2024-1', status: 'approved', submitDate: '2025-10-01', proofMaterials: [] },
])
...
function remove(row: CompetitionItem) {
  ElMessageBox.confirm(`确定删除 "${row.competitionName}" 的申报记录吗？`, '提示', { type: 'warning' })
    .then(() => {
  ...
  >
    <template #form>
      <el-form :model="form" label-width="120px">
        <el-form-item label="竞赛名称" required><el-input v-model="form.competitionName" placeholder="请输入竞赛名称" /></el-form-item>
        ...
        <el-form-item label="获奖时间" required><el-date-picker v-model="form.awardDate" type="month" placeholder="选择年月" /></el-form-item>
    ...
    <template #columns>
      ...
      <el-table-column label="竞赛类型" width="120">
        <template #default="{ row }">
          <DictColumn :value="(row as CompetitionItem).competitionType" :options="COMPETITION_TYPES" />
        </template>
      </el-table-column>
      ...
      <el-table-column label="状态" width="100">
        <template #default="{ row }"><StatusTag :status="(row as CompetitionItem).status" size="small" /></template>
      </el-table-column>
    </template>
  </ApplicationFormRecord>
</template>
