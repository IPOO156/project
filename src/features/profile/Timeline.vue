...
function getTypeColor(type: string): string { return getColor(type) ?? '#909399' }
function getTypeLabel(type: string): string { return getLabel(type) }
function getEventsForSemester(semester: string) { return timelineData.value.filter(e => e.semester === semester) }
        ...
        <template v-for="semester in semesterGroups" :key="semester">
          <div v-if="getEventsForSemester(semester).length > 0" class="timeline__semester">
            ...
            <div class="timeline__events">
              <div v-for="event in getEventsForSemester(semester)" :key="event.id" class="timeline__event">
                <div class="timeline__event-icon" :style="{ background: `${getTypeColor(event.type)}20`, color: getTypeColor(event.type) }">
                ...
                <div class="timeline__event-body">
                  <div class="timeline__event-header">
                    ...
                    <el-tag :color="getTypeColor(event.type)" size="small" effect="plain">{{ getTypeLabel(event.type) }}</el-tag>
                  </div>
...
.timeline__content { padding: 8px 0; }
.timeline__semester { position: relative; padding-left: 20px; margin-bottom: 32px;
  &-header { display: flex; align-items: center; gap: 12px; margin-bottom: 20px; }
  &-dot { width: 14px; height: 14px; border-radius: 50%; background: var(--el-color-primary); border: 3px solid var(--el-color-primary-light-5); margin-left: -27px; flex-shrink: 0; }
  &-label { font-size: 18px; font-weight: 600; color: var(--el-text-color-primary); }
}
.timeline__events { display: flex; flex-direction: column; gap: 16px; }
.timeline__event { display: flex; gap: 16px; padding: 16px; background: var(--el-fill-color-lighter); border-radius: 10px; transition: transform 0.2s, box-shadow 0.2s;
  ...
  &-title { font-size: 15px; font-weight: 600; color: var(--el-text-color-primary); }
  &-desc { font-size: 14px; color: var(--el-text-color-secondary); margin-bottom: 4px; }
