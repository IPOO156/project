...
/**
 * 获取提交记录列表
 * 后端就绪后替换为：return request.get<PaginatedData<SubmissionRecord>>('/submissions', { params })
 */
export function getSubmissionRecords(filters?: SubmissionFilters): Promise<SubmissionRecord[]> {
...
/**
 * 撤回提交记录
 * 后端就绪后替换为：return request.post(`/submissions/${id}/withdraw`)
 */
export function withdrawSubmission(id: string): Promise<void> {
  return new Promise((resolve) => {
    setTimeout(() => {
      if (cachedRecords) {
        const idx = cachedRecords.findIndex((r) => r.id === id)
        if (idx >= 0) cachedRecords[idx] = { ...cachedRecords[idx], status: 'draft' }
      }
      resolve()
    }, 200)
  })
...
/**
 * 提交申报申请
 * 后端就绪后替换为：return request.post('/submissions', data)
 */
export function submitApplication(data: Record<string, any>): Promise<SubmissionRecord> {
  return new Promise((resolve) => {
    setTimeout(() => {
      const record: SubmissionRecord = {
        id: String(Date.now()),
        type: data.type ?? 'competition',
        typeLabel: data.typeLabel ?? '未知类型',
        title: data.title ?? data.competitionName ?? data.projectName ?? '申报',
