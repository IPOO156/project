...
| RecordDetailDialog | `src/shared/ui/RecordDetailDialog.vue` | 申报/奖项记录「查看」详情弹窗（仅展示关键字段 + 中文标签） | `visible: boolean`, `record?: Record<string, any> \| null` / `update:visible`, `close` | `<RecordDetailDialog :visible="page.detailVisible" :record="page.detailRecord" @close="page.closeDetail" />` | 前端团队 |
